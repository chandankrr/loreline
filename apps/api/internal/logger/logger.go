...
// NewLogger creates a new logger with specified level (backward compatibilty)
...
// NewLoggerWithConfig creates a logger with full config (backward compatibilty)
...
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	// Include stack traces for errors in developement
	if !cfg.IsProduction() {
...
// WithTraceContext add New Relic transaction context to logger
