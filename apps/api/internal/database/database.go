...
import (
	...
	loggerConfig "github.com/chandankrr/loreline/internal/logger"
...
// DBTX defines the common operations shared by *pgxpool.Pool and pgx.Tx
type DBTX interface {
...
func (mt *multiTracker) TraceQueryStart(ctx context.Context, conn *pgx.Conn, data pgx.TraceQueryStartData) context.Context {
...
func (mt *multiTracker) TraceQueryEnd(ctx context.Context, conn *pgx.Conn, data pgx.TraceQueryEndData) {
	for _, tracer := range mt.tracers {
		if t, ok := tracer.(interface {
...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerConfig.LoggerService) (*Database, error) {
	...
	if cfg.Primary.Env == "local" {
		...
		if pgxPoolConfig.ConnConfig.Tracer != nil {
			...
			pgxPoolConfig.ConnConfig.Tracer = &multiTracker{
